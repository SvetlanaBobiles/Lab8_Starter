...
function initializeServiceWorker() {
  ...
  // B2. TODO - Listen for the 'load' event on the window object.
    ...
    console.log("Service worker is not supported in current browser.");
  }
}
...
async function getRecipes() {
  ...
  return new Promise(fetchRecipes);
  ...
  async function fetchRecipes(resolve, reject){
    ...
    for(let url of RECIPE_URLS){
      ...
      try{
        ...
        let dataURL = await fetchedURL.json();

        // A8. TODO - Add the new recipe to the recipes array

        recipesArr.push(dataURL);
        ...
        //            resolve() method.
        if(RECIPE_URLS.length == recipesArr.length){
        ...
        console.error('Error while trying to fetch recipe', error);
